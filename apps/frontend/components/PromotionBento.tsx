...
const promotions = [
  ...
  },
  {
    ...
    description: "ซื้อแพ็กเกจเพิ่ม เพื่อเพิ่มทรัพย์ของคุณได้มากกว่า 1000 รายการ",
...
export default function PromotionBento() {
  ...
  const handlePromotionClick = (id: number) => {
    if (id === 2) {
      // สำหรับ MRT ให้เลื่อนไปที่เซกชันสถานี
      const element = document.getElementById('nearby-stations');
      if (element) {
        element.scrollIntoView({ behavior: 'smooth' });
        // ส่งสัญญาณบอก NearbyStations ให้เปลี่ยนเป็น MRT
        window.dispatchEvent(new CustomEvent('filterStations', { detail: 'MRT' }));
        router.push('/?stationType=MRT', { scroll: false });
      }
    }
  };
      ...
      >
        {promotions.map((promo) => (
          <div 
            ...
            className={promo.className + " block"}
          >
            <motion.div
              variants={itemVariants}
              whileHover={{ scale: 1.01 }}
              className={`group relative overflow-hidden rounded-[2.5rem] border border-white/5 backdrop-blur-sm transition-all duration-500 hover:border-white/20 hover:shadow-2xl hover:shadow-black/60 h-full w-full cursor-pointer`}
            >
